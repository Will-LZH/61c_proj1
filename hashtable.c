...
void insertData(HashTable *table, void *key, void *data) {
  ...
  // 3. Append to the linked list or create it if it does not yet exist. 
	int divide=table->size;
	int locate=(table->hashFunction(key))%divide;
	struct HashBucket *nHB=(struct HashBucket *)malloc(sizeof(struct HashBucket));
	nHB->next=table->data[locate];
	nHB->data=data;
	nHB->key=key;
	table->data[locate]=nHB;
...
void *findData(HashTable *table, void *key) {
  ...
  // 2. Walk the linked list and check for equality with table->equalFunction.
	...
	int locate=(table->hashFunction(key))%divide;
	int test=0;
	struct HashBucket *temp=table->data[locate];
	while(temp!=NULL){
		if((table->equalFunction)(key, temp->key) != 0){
			test=1;
			break;
		}
		temp=temp->next;
